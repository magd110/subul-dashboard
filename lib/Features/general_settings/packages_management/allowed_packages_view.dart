...
class AllowedPackagesView extends StatelessWidget {
  @override
    ...
    return BlocBuilder<ParcelCubit, ParcelState>(
      builder: (context, state) {
        if (state is ParcelLoading) {
          ...
          final allowedItems = state.parcels
    .where((item) => item.isAllowed == 1)
    .toList(); 

          return RefreshIndicator(
            onRefresh: () => context.read<ParcelCubit>().fetchParcels(),
            child: PackageItems(
              title: "محتوى الطرود المسموح",
              items: allowedItems,
              image: AssetsData.greenCircle,
             onAddPressed: () {
  showDialog(
    context: context,
